...
def setup_iptables(ula_address, external_interface):
    print(f"{Fore.CYAN}Setting up iptables for IP masquerading and allowing necessary traffic...")
    os.system("sudo ip6tables --flush")
    os.system("sudo ip6tables --table nat --flush")
    os.system("sudo ip6tables --delete-chain")
    os.system("sudo ip6tables --table nat --delete-chain")
    ...
    os.system("sudo ip6tables -A INPUT -p udp --dport 53 -j ACCEPT")
    os.system("sudo ip6tables -A INPUT -p tcp --dport 53 -j ACCEPT")

    # Allow DHCPv6 (ports 546 and 547) traffic
    os.system("sudo ip6tables -A INPUT -p udp --dport 547 -j ACCEPT")
    os.system("sudo ip6tables -A INPUT -p udp --dport 546 -j ACCEPT")

    # Allow ICMPv6 (for RA and other IPv6 functions)
    os.system("sudo ip6tables -A INPUT -p ipv6-icmp -j ACCEPT")
    os.system("sudo ip6tables -A OUTPUT -p ipv6-icmp -j ACCEPT")

    print(f"{Fore.GREEN}iptables rules for IPv6 configured.")
...
def main():
    ...
    if args.restore:
        ...
        external_interface = 'eth0'  # Adjust to the correct external interface
