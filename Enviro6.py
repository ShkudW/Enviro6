...
def ping_multicast_address(iface):
    # Function to ping the multicast address in a loop
    while True:
        os.system(f"ping6 -I {iface} ff02::1 -c 1 > /dev/null 2>&1")
        time.sleep(1)

def sniff_ndp_packets(iface):
    print(f"{Fore.CYAN}Starting active NDP sniffing on interface {iface}...")
    seen_addresses = set()

    # Start the ping thread
    ping_thread = threading.Thread(target=ping_multicast_address, args=(iface,))
    ...
    def handle_packet(pkt):
        ...
        elif UDP in pkt and pkt[UDP].sport == 5355 and pkt[UDP].dport == 5355:
            print(f"{Fore.YELLOW}LLMNR Query: {pkt[IPv6].src} -> {pkt[IPv6].dst} : {pkt[DNSQR].qname.decode()}")

    sniff(iface=iface, prn=handle_packet, filter="icmp6 or udp port 5355")
...
def main():
    ...
    if not args.iface and not args.restore:
